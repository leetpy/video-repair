...
一个免费的本地视频修复工具原型：选择 AVI 或其他本地视频，调用 FFmpeg 和 Real-ESRGAN-ncnn-vulkan 修复并输出视频。
...
- 默认处理模式是 `极速 AI（轻量实验）`，目标为 1080p：会把 AI 输入限制在 960×540 以内，自动使用轻量 x2 视频模型和 `1:2:2` 流水队列。它比真实场景 x4 模型快得多，但处理真人或复杂纹理时可能损失细节。
- `AI 高画质` 使用真实场景模型，仍然是逐帧重型 AI 推理；长视频可能需要数十小时。
- `快速修复` 会跳过抽帧和 Real-ESRGAN，只执行去隔行、降噪、轻度锐化、缩放及编码，适合已经接近目标分辨率的视频。
- Windows 会依次尝试 NVIDIA NVENC、Intel QSV 和 AMD AMF 硬件编码；不可用或编码失败时自动回退到 `libx264 medium`。
- `GTX 1060 快速` 保留原尺寸 AI x2，再由 FFmpeg 缩放到目标分辨率。
...
- 大视频会很慢，建议先用短片段测试。
- `预览帧数` 默认为 20，可在界面修改；预览会使用当前参数生成独立视频，完成后可直接在本地页面播放，确认效果后再处理完整视频。
